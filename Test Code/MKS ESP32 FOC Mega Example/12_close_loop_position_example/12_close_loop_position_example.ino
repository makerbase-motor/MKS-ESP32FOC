// MKS ESP32 FOC Mega Current Close Loop Position Contorl Example Library: SimpleFOC 2.2.1 Hardware: MKS ESP32 FOC PLUS & MKS AS5600

// ！！！Cautions！！！
// ① Enter "A" followed by a number via the serial port to set the position of the M0 motor; for example, entering "A3.14" rotates the M0 motor to the 180° position (in radians).
// ② When using your own motor, be sure to update the default pole pair count—specifically the value within `BLDCMotor(7)`—to match the specifications of your motor.
