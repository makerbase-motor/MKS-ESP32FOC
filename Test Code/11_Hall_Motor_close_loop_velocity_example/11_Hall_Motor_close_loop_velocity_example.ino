  /**
MKS ESP32 FOC Hall Closed Loop Velocity Control Example Library：SimpleFOC 2.2.1 Hardware：MKS ESP32 FOC V1.0
Enter in the serial monitor：T+velocity, can make two motors rotate in a closed loop
For example, if both motors are to rotate at a speed of 20, enter their radian system: T20
When using your own motor, please remember to modify the default pole pair number, 
that is, the values in BLDCMotor() and HallSensor(), and set it to your own pole pair number.
The default power supply voltage set by the program is 24V. Please remember to 
modify the values in the voltage_power_supply and voltage_limit variables when using other voltages.
